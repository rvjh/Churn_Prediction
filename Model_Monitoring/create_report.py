...
def run_evidently(ref_data, recent_data):
    ...
    mapping = ColumnMapping(prediction="prediction",
                            target="target",
                            numerical_features=['customer_age', 'dependent_count', 'education_level', 'income_category',
...
def main():
    ...
    if (len(ref_data) > 0 and len(recent_data) > 0):
        ...
        log.warning(
            f"Ignoring generating report because: ref_data size: {len(ref_data)}, recent_data size: {len(recent_data)}")
